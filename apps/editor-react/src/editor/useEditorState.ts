...
export function createInitialEditorState(restoredState: PersistedEditorState | null): EditorState {
  ...
  const initialUiState =
    restoredState?.ui ?? sanitizePersistedUiState(
      {
...
export function createEditorStore(restoredState: PersistedEditorState | null = readPersistedEditorState()) {
...
export function useEditorState() {
  ...
  const selectors = useMemo<EditorSelectors>(() => {
    const selectedScene =
      state.project.scenes.find((scene) => scene.id === state.selectedSceneId) ?? state.project.scenes[0];
    ...
    const currentSceneIndex = state.project.scenes.findIndex((scene) => scene.id === selectedScene.id);
